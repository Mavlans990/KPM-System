<?php
  ...
  include "lib/appcode.php";
    ...
    if (isset($_POST['save'])) {
      $valid=1;
      if ($_POST['id']=="") {
        $query=mysql_query("INSERT INTO dietery(id, kategori, filter) VALUES ('','".mysql_real_escape_string($_POST['kategori'])."','".mysql_real_escape_string($_POST['filter'])."')");
        if (!$query) {
          $valid=0;
          $msg="ERROR : Insert Data Failed";
        }
      } else {
        $query=mysql_query("UPDATE dietery SET kategori='".$_POST['kategori']."',filter='".$_POST['filter']."' WHERE id='".$_POST['id']."'");
        if (!$query) {
          $valid=0;
    ...
    if (isset($_POST['delete'])) {
      $valid=1;
      $query=mysql_query("DELETE FROM dietery WHERE id='".$_POST['id_hapus']."'");
      if (!$query) {
        $valid=0;
      ...
      <!-- Main Content -->
       ...
       <?php //include "part/topbar.php"; ?>
        ...
        <div class="container-fluid mt-xl-15 mt-sm-30 mt-5">
          ...
          <!-- Page Heading -->
          <h1 class="h3 mb-2 text-gray-800">Category Master</h1>
              ...
              <div class="table-responsive">
                <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15" id="datatables1">
                  ...
                  <tbody>
                    <?php
                    $n=1;
                    $sql=mysql_query("SELECT * FROM dietery ORDER BY id ASC");
