...
if (isset($_POST['save'])) {
    ...
    $nama_ukuran = mysqli_real_escape_string($conn, $_POST['nama_kategori']);

    $select_bahan = "SELECT * FROM tb_ukuran WHERE id_ukuran = '" . $id . "'";
    $query_bahan = mysqli_query($conn, $select_bahan);
    $jum_bahan = mysqli_num_rows($query_bahan);

    if ($jum_bahan < 1) {
        $insert = "INSERT INTO tb_ukuran(
    ...
    echo "<script type='text/javascript'>alert('" . $msg . "');window.location='master_ukuran.php';</script>";
}

if (isset($_POST['delete'])) {
    $valid = 1;
    $query = mysqli_query($conn, "DELETE FROM tb_ukuran WHERE id_ukuran ='" . mysqli_real_escape_string($conn, $_POST['id_hapus']) . "'");
    if (!$query) {
        $valid = 0;
    ...
    echo "<script type='text/javascript'>alert('" . $msg . "');window.location.href='master_ukuran.php';</script>";
}
...
<body id="page-top">
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3" style="text-transform: none;">
                            <div class="form-inline">
                                <h3 class="mr-2">Master Ukuran</h3>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15" id="datable_1">
                                    ...
                                    <tbody>
                                        <?php
                                        $n = 1;
                                        $sql = mysqli_query($conn, "SELECT * FROM tb_ukuran ORDER BY id_ukuran ASC");
                                        while ($row = mysqli_fetch_array($sql)) {
                                            ...
                                            echo '
                                                ...
                                                data-id_kategori="' . $row['id_ukuran'] . '"
                                                data-kode="' . $row['id_ukuran'] . '"
                                                data-nama_kategori="' . $row['nama_ukuran'] . '">
                                                <i class="fa fa-edit"></i> Ubah</a>
                                                <a href="#" class="btn btn-small text-danger hapus_button" data-toggle="modal" data-target="#DeleteBrandModal"
                                                  data-id_hapus="' . $row['id_ukuran'] . '">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Master Ukuran</h5>
    ...
    <div class="modal fade" id="DeleteBrandModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Hapus Ukuran Kain</h5>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
                    <div class="modal-body">
                        ...
                        Anda yakin ingin menghapus ukuran ini ?
                    </div>
                    <div class="modal-footer">
                        <button class="btn btn-secondary" type="button" data-dismiss="modal">Batal</button>
