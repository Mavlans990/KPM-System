...
$tambah_baru = '<a href="master_barang_keluar_edit.php?id_transaksi=new" class="btn btn-primary form-control-sm"><i class="fa fa-plus"></i> Tambah Baru</a>';
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3" style="text-transform: none;">
                            <div class="form-inline">
                                <h3 class="mr-2">Master Barang Keluar</h3>
                                <?= $tambah_baru; ?>
                            </div>
                        </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15">
                                    ...
                                    <tbody>
                                        ...
                                        $sql_get_barang_keluar = mysqli_query($conn, "SELECT a.* FROM tb_barang_keluar a WHERE a.id_transaksi LIKE '%SO%' AND by_user_pajak = '" . $_SESSION['jenis_pajak'] . "' " . $where_tgl . " " . $where_customer . "  GROUP BY a.id_transaksi ORDER BY a.id_transaksi DESC LIMIT 50");
                                        while ($row_barang_keluar = mysqli_fetch_array($sql_get_barang_keluar)) {
                                            ...
                                            if ($status_keluar == "s") {
                                                $sts = '<span class="text-success">Close</span>';
                                            }
                                            ...
                                            $link_edit = "master_barang_keluar_edit.php?id_transaksi=" . $id_transaksi;
                                            $edit_text  = "<i class='fa fa-pencil'></i> Ubah";
                                            if ($status_keluar == "s") {
                                                $btn_edit = "btn-primary";
                                                $link_edit = "master_barang_keluar_edit.php?id_transaksi=" . $id_transaksi . "&view=detail";
                                            ...
                                            if($row_barang_keluar["jenis_transaksi"] == "penjualan"){
                                                $jns = "(Penjualan) ";
                                            }elseif($row_barang_keluar["jenis_transaksi"] == "return"){
                                                $jns = "(Return) ";
                                            }else{
                                                $jns = " ";
                                            }
                                            ...
                                            echo '
                                                <tr>
                                                    ...
                                                    ';
                                                    if($jatuh_tempo==0){
                                                        echo'<td class="text-center fs-11">CASH</td>';
                                                    }else{
                                                        echo '<td class="text-center fs-11">' . date("d-m-Y", strtotime($tgl_jatuh_tempo)) . '</td>';
                                                    }
                                                    
                                                    echo '
                                                    <td class="text-center fs-11">' . $nama_customer . '</td>
                                                    <td class="fs-11">'. $jns . $keterangan . '</td>
