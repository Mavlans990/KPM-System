...
if (isset($_POST['delete'])) {
    ...
    if ($valid == 1) {
        $query_del = mysqli_query($conn, "DELETE FROM m_user WHERE id_user='" . mysqli_real_escape_string($conn, $_POST['id_hapus']) . "' ");
        if (!$query_del) {
            $valid = 0;
                            ...
                            <div class="row">
                                <div class="col-sm-10">
                                    <h5 class="hk-sec-title">Master User Login
                                        <a href="master_user_edit.php?id_user=new" class="btn btn-success btn-sm btn-wth-icon icon-wthot-bg btn-rounded icon-right"><span class="btn-text">Add New</span><span class="icon-label"><i class="fa fa-angle-right"></i> </span></a>
                                    </h5>
                                </div>
                                <!-- <div class="col">
                                    <h5 class="hk-sec-title">
                                        <a href="print/master_user_login_print.php"
                                    ...
                                    <div class="table-wrap table-scroll">
                                        <table id="datable_2" class="table table-hover w-100 display">
                                            ...
                                            <mody>
                                                ...
                                                $no = 1;
                                                // Panggil m.user
                                                $query_get = mysqli_query($conn, "SELECT u.nm_user, 
                                                                                    u.id_user,
                                                                                    u.group_user,
                                                                                    u.id_branch
                                                                                FROM m_user u
                                                                                -- LEFT JOIN m_branch b ON b.m_branch_id = u.id_branch
                                                ...
                                                while ($row_get = mysqli_fetch_array($query_get)) {
                                                    ...
                                                    $no_urut = 0;
                                                    $to = "Pusat";
                                                    $query_get_to = mysqli_query($conn, "SELECT id_cabang,nama_cabang FROM tb_cabang ");
                                                    while ($row_to = mysqli_fetch_array($query_get_to)) {
                                                        if (strpos($id_branch, $row_to['id_cabang']) > -1) {
                                                            if ($no_urut == 0) {
                                                                $to = $row_to['nama_cabang'];
                                                                $no_urut++;
                                                            } else {
                                                                $to = $to . ',' . $row_to['nama_cabang'];
                                                                $no_urut++;
                                                            }
                                                        }
                                                    }
                                                    // $group = $row_get['group_user'];
                                                    // if($group == "super"){
                                                    //     $group = "super admin";
                                                    // }
                                                ...
                                                ';
                                                    ...
                                                    echo '
                                                    <td>' . $no . '</td>    
                                                    <td>' . $row_get['id_user'] . '</td>
                                                    <td>' . $row_get['nm_user'] . '</td>
                                                    <td>' . $to . '</td>
                                                    <td>
                                                        <a href="master_user_edit.php?id_user=' . $row_get['id_user'] . '" class="btn btn-xs btn-icon btn-warning btn-icon-style-1" >
