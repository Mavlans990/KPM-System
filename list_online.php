...
if (isset($_POST['save'])) {
    $valid = 1;
    if ($_POST['id'] == "") {
        $kategori = str_replace(" ", "_", strtolower($_POST['kategori']));
        $query = mysqli_query($conn, "INSERT INTO category(kategori, filter) VALUES ('" . mysqli_real_escape_string($conn, ucfirst($_POST['kategori'])) . "','" . mysqli_real_escape_string($conn, $kategori) . "')");
        if (!$query) {
            $valid = 0;
        ...
        $kategori = str_replace(" ", "_", strtolower($_POST['kategori']));
        $query = mysqli_query($conn, "UPDATE category SET kategori='" . mysqli_real_escape_string($conn, ucfirst($_POST['kategori'])) . "', filter='" . mysqli_real_escape_string($conn, $kategori) . "' WHERE id='" . mysqli_real_escape_string($conn, $_POST['id']) . "'");
        if (!$query) {
            $valid = 0;
...
if (isset($_POST['delete'])) {
    $valid = 1;
    $query = mysqli_query($conn, "DELETE FROM category WHERE id='" . mysqli_real_escape_string($conn, $_POST['id_hapus']) . "'");
    if (!$query) {
        $valid = 0;
        ...
        <div class="hk-wrapper hk-vertical-nav">
            ...
            <div class="hk-pg-wrapper">
                ...
                <div class="container-fluid mt-xl-15 mt-sm-30 mt-5">
                    ...
                    <!-- Page Heading -->
                    <h1 class="h3 mb-2 text-gray-800">Daftar Pesanan Online</h1>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15" id="datatables1">
                                    ...
                                    <tbody>
                                        <?php
                                        $select_sj1 = "SELECT * FROM sj1 ORDER BY tgl_sj ASC";
                                        $query_sj1 = mysqli_query($conn, $select_sj1);
                                        while ($row_sj1 = mysqli_fetch_array($query_sj1)) {
                                            echo '
                                                <tr>
                                                    <td>' . date("d/m/Y", strtotime($row_sj1['tgl_sj'])) . '</td>
                                                    <td>' . $row_sj1['id_sj1'] . '</td>
                                                    <td>' . $row_sj1['nm_cust'] . '</td>
                                                    <td>
                                                        <a href="order.php?order=' . $row_sj1['id_sj1'] . '" class="text-info"><i class="fa fa-table"></i> Lihat</a>
                                                    </td>
                                                </tr>
                                            ';
                                        }
