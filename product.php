...
if (isset($_POST['filter_motor'])) {
  $_SESSION['motor'] = $_POST['motor'];
  header("location:product.php");
}

if (isset($_POST['reset_motor'])) {
...
<head>
  ...
  <script>
    function motor() {
      ...
      $.ajax({
        type: "POST",
        url: "ajax/ajax_jenis_motor.php",
        data: {
          "merk_motor": merk_motor
        },
        cache: true,
        success: function(result) {
          $(".jenis_motor").html(result);
        }
      });
    }
  </script>
        ...
        <div class="container-fluid mt-xl-15 mt-sm-30 mt-5">
          ...
          if (isset($_GET['act'])) {
            if ($_GET['act'] == 'add' || $_GET['act'] == 'edit') {
              ...
              if ($_GET['act'] == 'edit') {
                $query = mysqli_query($conn, "SELECT * FROM product WHERE id_product='" . mysqli_real_escape_string($conn, $_GET['id']) . "'");
                while ($row = mysqli_fetch_array($query)) {
                  $id_product = $row['id_product'];
                  $merk = $row['merk'];
                  $jenis_motor = $row['jenis_motor'];
                ...
                <div class="card-body">
                  <form action="modul/product_module.php" enctype="multipart/form-data" method="POST">
                    ...
                    <div class="form-group">
                      ...
                      <select name="merk_motor" id="" class="form-control merk_motor">
                        <?php
                        ...
                        while ($row_merk = mysqli_fetch_array($query_merk)) {
                          $select = "";
                          if ($row_merk['filter'] == $merk) {
                            $select = "selected";
                          }
                          echo '<option value="' . $row_merk['filter'] . '" ' . $select . '>' . $row_merk['merk_motor'] . '</option>';
                        }
                        ?>
                      </select>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="jenis_motor" id="" class="form-control jenis_motor">
                        <?php
                        $select_motor = "SELECT * FROM tb_motor ORDER BY id ASC";
                        $query_motor = mysqli_query($conn, $select_motor);
                        while ($row_motor = mysqli_fetch_array($query_motor)) {
                          $select = "";
                          if ($jenis_motor == $row_motor['filter']) {
                            $select = "selected";
                          }
                          echo "<option value='" . $row_motor['filter'] . "' " . $select . ">" . $row_motor['nama_motor'] . "</option>";
                        }
                        ?>
                      </select>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <select name="jenis_bahan" id="" class="form-control">
                        <?php
                        $select_bahan = "SELECT * FROM tb_bahan ORDER BY id ASC";
                        $query_bahan = mysqli_query($conn, $select_bahan);
                        while ($row_bahan = mysqli_fetch_array($query_bahan)) {
                          $select = "";
                          if ($jenis_bahan == $row_bahan['filter']) {
                            $select = "selected";
                          }
                          echo "<option value='" . $row_bahan['filter'] . "' " . $select . ">" . $row_bahan['nama_bahan'] . "</option>";
                        }
                        ?>
                      </select>
                    </div>
                    ...
                    <!-- <div class="form-group">
                      ...
                      <input type="number" class="form-control" name="berat" id="" value="<?php echo $berat; ?>">
                    </div> -->
                    ...
                    <!-- <div class="form-group">
                      ...
                      <select name="tipe" id="" class="form-control">
                        <option value="decal" <?php if ($tipe == "decal") {
                                                echo "selected";
                                              } ?>>Decal</option>
                        <option value="testimoni" <?php if ($tipe == "testimonil") {
                                                    echo "selected";
                                                  } ?>>Testimoni</option>
                      </select>
                    </div> -->
                      ...
                      <div class="form-group form-inline">
                        ...
                        if (isset($_SESSION['grup'])) {
                          if ($_SESSION['grup'] == "super") {
                            echo '<a href="product.php?act=add" class="mr-3"><i class="fa fa-plus"></i> Tambah Baru</a>';
                          }
                        }
                        ?>
                        <form action="" method="post">
                          <select name="motor" id="" class="form-control">
                            <option value="">-- Jenis Motor --</option>
                            <?php
                            $select_motor = "SELECT * FROM tb_motor ORDER BY nama_motor ASC";
                            $query_motor = mysqli_query($conn, $select_motor);
                            while ($row_motor = mysqli_fetch_array($query_motor)) {
                              $select = "";
                              if (isset($_SESSION['motor'])) {
                                if ($_SESSION['motor'] == $row_motor['filter']) {
                                  $select = "selected";
                                }
                              }
                              echo '
                              <option value="' . $row_motor['filter'] . '" ' . $select . '>' . $row_motor['nama_motor'] . '</option>
                            ';
                            }
                            ?>
                          </select>
                          <button type="submit" class="btn btn-primary" name="filter_motor">Pilih</button>
                        </form>
                        <?php
                        if (isset($_SESSION['motor'])) {
                        ?>
                          <form action="" method="post">
                            <button type="submit" class="btn btn-danger ml-1" name="reset_motor">Reset</button>
                          </form>
                        <?php
                        }
                        ?>
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15" id="datable_1">
                        ...
                        <tbody>
                          <?php
                          $n = 1;
                          $sql = mysqli_query($conn, "SELECT * FROM product WHERE jenis_motor = '" . $_SESSION['motor'] . "' AND tipe = 'decal' ORDER BY id_product ASC");
                          while ($row = mysqli_fetch_array($sql)) {
                            $select_bahan = "SELECT * FROM tb_bahan WHERE filter = '" . $row['jenis_bahan'] . "'";
                            $query_bahan = mysqli_query($conn, $select_bahan);
...
<script type="text/javascript">
  ...
  $(document).on("change", ".merk_motor", function() {
    var merk_motor = $(".merk_motor").val();
    $.ajax({
      type: "POST",
      url: "ajax/ajax_jenis_motor.php",
