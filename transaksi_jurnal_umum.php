...
if(isset($_POST['add_jurnal'])){
    ...
    $query_get_sum = mysqli_query($conn,"SELECT SUM(debit) as debit_total,
                                        SUM(kredit) as kredit_total 
                                        FROM tb_jurnal_umum
                                        where no_transaksi = '".$no_transaksi."'");
        $row_get_sum = mysqli_fetch_array($query_get_sum);
        $total_kredit = $row_get_sum['kredit_total'];
        $total_debit = $row_get_sum['debit_total'];
    if($total_debit == $total_kredit){
    // Insert
    if($id_transaksi_filter == "new" || $id_transaksi_filter == $id_user){
        
        $urutan = "0";
        $query = mysqli_query($conn,"SELECT max(no_transaksi) as kodeTerbesar FROM tb_jurnal_umum WHERE NOT no_transaksi IN (SELECT id_user FROM tb_user)");
        ...
        $query_set_jurnal = "UPDATE tb_jurnal_umum SET no_transaksi = '".$no_transaksi."' ,total_debit = '".$total_debit."' ,total_kredit = '".$total_kredit."'  WHERE no_transaksi = '".$id_user."'";
    ...
    if($id_transaksi_filter !== "new" || $id_transaksi_filter !== $id_user){
        ...
        $query_set_jurnal = "UPDATE tb_jurnal_umum SET memo = '".$memo."' ,total_debit = '".$total_debit."' ,total_kredit = '".$total_kredit."' WHERE no_transaksi = '".$no_transaksi."'";
        if($set_jurnal = mysqli_query($conn,$query_set_jurnal)){
            echo "<script type='text/javascript'>alert('Simpan Data Berhasil')</script>";
    ...
    // Update End
        ...
        echo "<script type='text/javascript'>alert('Total Debit dan Total Kredit tidak sama')</script>";
    }
...
if(isset($_POST['delete'])){
    ...
    $msg = "Delete Jurnal Umum #".$id_hapus." Gagal ";
    $sql_del = "DELETE FROM tb_jurnal_umum WHERE no_transaksi = '".$id_hapus."'";
    if($query_del = mysqli_query($conn,$sql_del)){
        $msg = "Delete Jurnal Umum #".$id_hapus." Berhasil";
    }
    echo "<script type='text/javascript'>alert('".$msg."')</script>";
    header ("location:transaksi_jurnal_umum_view.php");
}
// Delete End
...
<body>
        ...
        <div class="hk-pg-wrapper">
            ...
            <div class="my-2">
             ...
             $query_get_jurnal = "SELECT DISTINCT no_transaksi,
                ...
                if($row_jurnal = mysqli_fetch_array($sql_get_jurnal)){
                    ...
                    $status_jurnal = $row_jurnal['status_jurnal'];                    
               ...
               <tr>
                       ...
                       <td style="font-size:11px;">IDR '.number_format($debit).'</td>
                       <td style="font-size:11px;">IDR '.number_format($kredit).'</td>
                       </tr>
                       ';
               ...
               echo'
                    ...
                    <div class="row mt-20">
                        ...
                        <div class="">
                            ...
                            <div class="col-sm-6">
                                ...
                                <p>IDR '.number_format($total_debit).'</p>
                            </div>                            
                            <div class="col-sm-6">
                                <p class="text-dark">Total Kredit</p>
                                <p>IDR '.number_format($total_kredit).'</p>
                            </div>
                            </div>
                        </div>
                        </div>
                    ...
                    <div class="row mt-10 justify-content-end">
                        <div class="col-sm-8">
                            <a href="print/jurnal_umum_print.php?id_jurnal='.$row_jurnal['no_transaksi'].'" class="btn btn-xs btn-info btn-wth-icon"><span class="icon-label"><i class="fa fa-print"></i></span><span class="btn-text">Print Transaksi</span></a>
                        </div>
                        <div class="col-sm-4">
                            ...
                            echo'
                            <a '.$hide_btn.' href="transaksi_jurnal_umum.php?id_jurnal='.$row_jurnal['no_transaksi'].'" class="btn btn-xs btn-warning btn-wth-icon mr-sm-2"><span class="icon-label"><i class="fa fa-pencil"></i></span><span class="btn-text">Ubah Transaksi</span></a>
                           <a '.$hide_btn.' href="#" class="btn btn-xs btn-danger btn-wth-icon hapus_button" data-toggle="modal" data-target="#DeleteBrandModal"
            ...
            <a href="transaksi_jurnal_umum_view.php" class="text-primary" style="font-size:15px;"> << Kembali </a>
               ...
               <div class="bg-primary-10 mt-10">
                    ...
                    ';
                    $query_get_akun = "SELECT  DISTINCT b.jurnal_umum_id,
                                                a.kode_akun,
                                                a.nm_akun,
                                                b.description,
                                                b.debit,
                    ...
                    <div class="row mt-5">
                        ...
                        <div class="col-sm-2">
                            <div class="input-group input-group-sm button0">
                    ...
                    <div class="row mt-10 justify-content-end">
                        <div class="col-sm-2">
                            <div class="form-group form-group-sm">
                            <button type="submit" class="add_jurnal btn btn-success btn-sm" style="width:10vw;"
                                        name="add_jurnal" style="color:white;">Save</button>
                            </div>
                        </div>
                    </div>
               </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
                ...
                $("input[id*='moneyfield']").tooltip("hide");
            ...
            $(document).on('keyup', "input[id*='moneyfield']", function (e) {
                ...
                var debit = $("#debit0"+id).val();
                var kredit = $("#kredit0"+id).val();
                debit = debit.split('.00').join('');
                kredit = kredit.split('.00').join('');
                $("input[id*='moneyfield']").tooltip({
                    placement: "bottom",
                    trigger: "focus",
                    title: "Debit dan kredit tidak boleh sama",
                });
                $("#"+name+"_moneyfield_"+id).tooltip("hide");
                if(debit == kredit){
                    $("#"+name+"_moneyfield_"+id).tooltip("show");
                    $("#debit_moneyfield_"+id).addClass("border-danger");
                    $("#kredit_moneyfield_"+id).addClass("border-danger");
                    $(".button"+id).hide();
                    
                    } else {
                        $("#"+name+"_moneyfield_"+id).tooltip("hide");
                        $("#debit_moneyfield_"+id).removeClass("border-danger");
                        $("#kredit_moneyfield_"+id).removeClass("border-danger");
                        $(".button"+id).show();
                    }
