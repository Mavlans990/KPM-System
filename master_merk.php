...
if (isset($_POST['save'])) {
    ...
    if ($_POST['id'] == "") {
        ...
        $query_merk = mysqli_query($conn, "
        SELECT *
        FROM tb_merk
        WHERE filter = '" . $merek_motor . "'");
        $jum_merk = mysqli_num_rows($query_merk);

        if ($jum_merk < 1) {
            $query = mysqli_query($conn, "INSERT INTO tb_merk(merk_mobil,filter) VALUES('" . mysqli_real_escape_string($conn, $_POST['merek_motor']) . "','" . mysqli_real_escape_string($conn, $merek_motor) . "')") or die(mysqli_error($conn));
            if (!$query) {
                $valid = 0;
        ...
        if ($jum_merk < 1) {
            $query = mysqli_query($conn, "UPDATE tb_merk SET merk_mobil = '" . mysqli_real_escape_string($conn, $_POST['merek_motor']) . "',filter = '" . mysqli_real_escape_string($conn, $merek_motor) . "' WHERE id = '" . mysqli_real_escape_string($conn, $_POST['id']) . "'");
    ...
    echo "<script type='text/javascript'>alert('" . $msg . "');window.location.href='master_merk.php';</script>";
}
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header" style="text-transform: none;">
                            <div class="form-inline">
                                <h3>Master Merek</h3>
                                <a href="#" data-toggle="modal" class="add_new btn btn-primary form-control-sm ml-2" data-target="#newBrandModal"><i class="fa fa-plus"></i> Tambah Baru</a>
                            </div>
                        </div>
                        <div class="card-body">
                                        ...
                                        while ($row = mysqli_fetch_array($sql)) {
                                            echo '
                                                ...
                                                <a href="#" class="btn btn-small text-info edit_button" data-toggle="modal" data-target="#newBrandModal"
                                                  data-id="' . $row['id'] . '"
                                                  data-merek_motor="' . $row['merk_mobil'] . '">
                ...
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="merek_motor" id="" class="form-control merek_motor" autocomplete="off">
                        </div>
                    </div>
