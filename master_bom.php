...
if (isset($_POST['delete'])) {
    ...
    $select_masuk = "SELECT * FROM tb_bom WHERE id_transaksi = '" . $id_transaksi . "'";
    $query_masuk = mysqli_query($conn, $select_masuk);
    ...
    $delete = mysqli_query($conn, "DELETE FROM tb_bom WHERE id_transaksi = '" . $id_transaksi . "'");
    if ($delete) {
    ...
    echo '
        <script>alert("' . $msg . '");window.location.href="master_bom.php";</script>
    ';
}
                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3" style="text-transform: none;">
                            <div class="form-inline">
                                <h3 class="mr-2">Master BOM</h3>
                                <a href="master_bom_edit.php?id_transaksi=new" class="btn btn-primary form-control-sm"><i class="fa fa-plus"></i> Tambah Baru</a>
                            </div>
                        </div>
                                ...
                                <form action="" method="post">
                                    ...
                                    <div class="row no-gutters">
                                        <div class="form-group form-inline">
                                            <input type="text" name="kode_bom" id="" class="form-control form-control-sm" value="<?= $kode_bom; ?>" required placeholder="Masukan Kode bom" list="list-kode">
                                            <datalist id="list-kode" class="kode-list">
                                                <?php 
                                                $sql = mysqli_query($conn,"SELECT kode_bom FROM tb_bom");
                                                while($row = mysqli_fetch_array($sql)){
                                                    echo '
                                                    <option value="'.$row['kode_bom'].'">
                                                    ';
                                                }
                                                ?>
                                            </datalist>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15">
                                    ...
                                    <tbody>
                                        ...
                                        $limit = "LIMIT 50";
                                        if (isset($_POST['search'])) {
                                            $where_kode= "AND kode_bom LIKE '%".$kode_bom."%'";
                                            $limit = "";
                                        ...
                                        $sql = mysqli_query($conn, "SELECT a.* FROM tb_bom a WHERE a.id_transaksi LIKE '%BOM%' " . $where_kode . "  GROUP BY a.id_transaksi ORDER BY id_transaksi DESC ".$limit."");
                                        while ($row = mysqli_fetch_array($sql)) {

                                            if ($row['status'] == "d") {
                                                $status = "<p class='text-warning'>Draft</p>";
                                                $ubah = '<a href="master_bom_edit.php?id_transaksi=' . $row['id_transaksi'] . '" class="btn btn-sm btn-success">
                                            ...
                                            echo '
                                                <tr>
                                                    ...
                                                    <td class="fs-11" style="vertical-align:top;">' . $row['kode_bom'] . '</td>
                                                    <td class="fs-11" style="vertical-align:top;">' . $row['nama_bom'] . '</td>
                                                    <td class="fs-11" style="vertical-align:top;">' . $row['keterangan'] . '</td>
