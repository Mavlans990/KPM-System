...
if ($row_barang_keluar = mysqli_fetch_array($sql_get_barang_keluar)) {
    $tanggal = date("d - F - Y", strtotime($row_barang_keluar['tgl_invoice']));
...
<body onload="window.print();">
    ...
    <table style="width:100%;">
        ...
        <tbody>
            ...
            $sql_get_barang_keluar = mysqli_query($conn, "SELECT SUM(polybag)  AS polybag, SUM(pcs)  AS pcs, id_sku FROM tb_packing WHERE id_invoice = '" . $id_transaksi . "' GROUP BY id_sku");
            while ($row_barang_keluar = mysqli_fetch_array($sql_get_barang_keluar)) {
                ...
                $sql_get_bahan_2 = mysqli_query($conn, "SELECT kode_sku FROM tb_sku WHERE id_sku = '" . $id_sku . "'");
