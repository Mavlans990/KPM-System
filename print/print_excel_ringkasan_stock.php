...
$branch_id = mysqli_real_escape_string($conn, $_GET['cabang']);
...
header("Content-Disposition: attachment; filename=Ringkasan Stock (Cabang : " . $data_cabang['nama_cabang'] . ") (" . date("d/m/Y", strtotime($_GET['tgl_from'])) . " - " . date("d/m/Y", strtotime($_GET['tgl_to'])) . ").xls");
...
<body>
    ...
    <!-- <center> -->
    <h3>RINGKASAN STOCK</h3>
    <h6><?= date('d/m/Y', strtotime($tgl_1)) ?> Sampai <?= date('d/m/Y', strtotime($tgl_1)) ?><br>
        Cabang : <?= $data_cabang['nama_cabang']; ?> <br></h6>
    <!-- </center> -->

    <table border="1" class="table table-hover table-sm w-100 display mt-15">
        <thead>
        ...
        while ($row_product = mysqli_fetch_array($query_get_product)) {
            ...
            if ($tgl_1 !== "" || $tgl_2 !== "") {
                ...
                // Beginning
                $sql_det = mysqli_query($conn, "SELECT i.id_cabang as branch, i.id_transaksi as nama,i.qty as masuk,'0' as keluar FROM tb_barang_masuk i JOIN tb_bahan p on i.id_product = p.id_bahan  WHERE p.id_bahan = '" . $row_product['id_bahan'] . "' and i.status = 's' " . $where_branch1 . " " . $where_beginning_in . "
                ...
                $sql_trf = mysqli_query($conn, "SELECT ai.stock_in as masuk,'0' as keluar FROM inv_adjust_in ai JOIN tb_bahan p on ai.id_product = p.id_bahan  WHERE p.id_bahan = '" . $row_product['id_bahan'] . "' and ai.id_inv_in LIKE 'SM-%' " . $where_branch3 . " AND ai.status_terima = 's' AND ai.inv_date BETWEEN '" . $tgl_1 . "' AND '" . $tgl_2 . "' 
