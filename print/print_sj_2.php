    ...
    <table style="width:100%;">
        <thead>
            <tr>
                ...
                <th style="border-top:1px solid #ccc;border-bottom:1px solid #ccc;border-left:1px solid #ccc;">No Work
                    Order</th>
        ...
        <tbody>
            ...
            $total_pcs = 0;
            $id_wo = '';
            // $select_wo = mysqli_query($conn,"SELECT id_transaksi,id_sku FROM tb_packing WHERE id_invoice = '".$id_transaksi."' GROUP BY id_transaksi,id_sku ");
            // echo "SELECT id_transaksi,id_sku FROM tb_packing WHERE id_invoice = '".$id_transaksi."' GROUP BY id_transaksi,id_sku <br>";
            // while($row_wo = mysqli_fetch_array($select_wo)){
            //     // echo $row_wo['id_transaksi'].' - '.$row_wo['id_sku'].'<br><br>';
            //     if($id_wo != '' && $id_wo != $row_wo['id_transaksi'] && $id_sku == $row_wo['id_sku']){
            //         // echo $id_wo.' & '.$row_wo['id_transaksi'].'<br>';
            //         // echo $id_sku.' & '.$row_wo['id_sku'].'<br>';
            //         echo 'SKU SAMA<br>';
            //         $id_wo = $id_wo .' & '.$row_wo['id_transaksi'];
            //     }else{
            //         echo 'SKU BEDA<br>';
            //         $id_wo = $row_wo['id_transaksi'];
            //         // echo $id_wo.'<br>';
            //     }
            //     echo $id_wo.'<br>';
            // }
            ...
            while ($row_barang_keluar = mysqli_fetch_array($sql_get_barang_keluar)) {
                ...
                // echo "SELECT id_wo,id_sku FROM tb_packing WHERE id_sku = '".$id_sku."'<br>";
                $sql_get_wo = mysqli_query($conn,"SELECT id_transaksi,id_sku FROM tb_packing WHERE id_sku = '".$id_sku."' AND id_invoice = '".$id_transaksi."' GROUP BY id_transaksi");
                $num_wo = mysqli_num_rows($sql_get_wo);
                    $id_wo = '';
                    while($row_wo = mysqli_fetch_array($sql_get_wo)){
                        if($id_wo != '' && strpos($id_wo, $row_wo['id_transaksi']) === false && $id_sku == $row_wo['id_sku'] ){
                            // echo $id_wo.'<br>';
                            $id_wo = $id_wo.'&'.$row_wo['id_transaksi'];
                        }else{
                            $id_wo = $row_wo['id_transaksi'];
                ...
                // <td style="text-align:center;">' . $id_sku . '</td>
                echo '
                    <tr>
        ...
        <form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
            <table style="width:15%;">
                ...
                <tr>
                    ...
                    <td style="text-align:center;"><input type="date" name="tgl_ubah" id=""
                            class="form-control form-control-sm " value="<?= $tgl; ?>"></td>
                    <td style="text-align:center;"><input type="hidden" name="id_sj" id=""
                            class="form-control form-control-sm " value="<?= $_GET['id_invoice'] ?>"></td>
