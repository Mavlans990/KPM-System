                                ...
                                <form action="" method="post">
                                    ...
                                    <div class="row no-gutters">
                                        <div class="form-group form-inline">
                                            <input type="text" name="tgl_from" id="" class="form-control form-control-sm" placeholder="Cari Nama Produk" >
                                            <input type="text" name="tgl_from" id="" class="form-control form-control-sm" placeholder="Cari Barcode" >
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15">
                                    ...
                                    <tbody>
                                        ...
                                        $sql = mysqli_query($conn, "SELECT * FROM tb_barang_masuk WHERE id_transaksi LIKE '%IN%' ORDER BY id_transaksi ASC LIMIT 50");
                                        while ($row = mysqli_fetch_array($sql)) {
                                            
                                            $no_id = $row['no_id'];

                                            $q_work = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM tb_work_order WHERE id_bahan_kain = '" . $row['id_bahan_kain'] . "' "));
                                            $stok = $q_work['qty_kain'];

                                            $q_inspek = mysqli_fetch_assoc(mysqli_query($conn, "SELECT barcode FROM tb_inspek WHERE barcode = '$no_id'"));

                                            $q_kain = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM tb_bahan_kain WHERE id_bahan_kain = '" . $row['id_bahan_kain'] . "'"));
                                            // var_dump($tb_inspek);
                                            if ($q_inspek['barcode'] != NULL) {
                                                $status = "<p class='text-success'>Terpakai</p>";
                                                $bruto = 0;
                                                $netto = 0;
                                            } else {
                                                $status = "<p class='text-primary'>Belum Terpakai</p>";
                                                $bruto = $row['bruto'];
                                                $netto = $row['netto'];
                                            }
                                            ...
                                            echo '
                                                <tr>
                                                <td class="fs-11" style="vertical-align:top;">' . $row['no_id'] . '</td>
                                                <td class="fs-11" style="vertical-align:top;">' .$q_kain['jenis_kain'] . ' - ' . $q_kain['warna'] . ' - ' . $q_kain['setting'] . ' - ' . $q_kain['gramasi'] . '</td>
                                                <td class="fs-11" style="vertical-align:top;">' . $row['lot'] . '</td>
                                        ...
                                        $total_stok = 0;
                                        $sql2 = mysqli_query($conn, "SELECT * FROM tb_barang_masuk WHERE id_transaksi LIKE '%IN%' GROUP BY id_bahan_kain ");
                                        while ($row2 = mysqli_fetch_array($sql2)) {
                                            $q_work = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM tb_work_order WHERE id_bahan_kain = '" . $row2['id_bahan_kain'] . "' "));
                                            $stok = $q_work['qty_kain'];
                                ...
                                <div class="input-group mb-3">
                                    <div class="input-group-prepend" style="text-align: right;">
                                        <span style="width:1020px;font-weight: 600;color:#273238;"  id="inputGroup-sizing-sm">Total Stok : <?= $total_stok ?> ROLL</span>
                                    </div>
                                </div>
                                <div class="input-group mb-3">
                                    <div class="input-group-prepend" style="text-align: right;">
                                        <span style="width:1020px;font-weight: 600;color:#273238"  id="inputGroup-sizing-sm">Total Bruto : <?= $total_bruto ?></span>
                                    </div>
                                </div>
                                <div class="input-group mb-3">
                                    <div class="input-group-prepend" style="text-align: right;">
                                        <span style="width:1020px;font-weight: 600;color:#273238"  id="inputGroup-sizing-sm">Total Netto : <?= $total_netto ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="modal-body">
                        <div class="input-group input-group-sm">
                            <div class="input-group-prepend">
                                <span class="form-control filled-input form-control-sm text-dark" id="inputGroup-sizing-sm">Cabang</span>
                            </div>
