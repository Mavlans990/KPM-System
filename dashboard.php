                    ...
                    <div class="card shadow mb-4">
                                        ...
                                        <div class="col-10">
                                            <div class="form-group" >
                                                <select name="tanggal" id="" class="form-control form-control-sm mt--5">
                                                    ...
                                                    ?>
                                                    <option <?php if ($tanggal == "Hari") {echo "selected";} ?> value="Hari">Today</option>
                                                    <option <?php if ($tanggal == "Minggu") {echo "selected";} ?> value="Minggu">This Week</option>
                                                    <option <?php if ($tanggal == "Bulan") {echo "selected";} ?> value="Bulan">This Month</option>
                        ...
                        if (isset($_POST['search'])) {
                            ...
                            if ($tanggal == "Hari") {
                                ...
                                $filter2 = "WHERE a.tgl_invoice = '". date("Y-m-d") ."' AND a.jatuh_tempo != 0";
                                $filter3 = "WHERE tgl_terima = '". date("Y-m-d") ."' AND metode != '1-1006'";
                            }
                            elseif ($tanggal == "Minggu") {
                                ...
                                $filter2 = "WHERE a.tgl_invoice BETWEEN '". $start_date ."' AND '". $end_date ."' AND a.jatuh_tempo != 0";
                                $filter3 = "WHERE tgl_terima BETWEEN '". $start_date ."' AND '". $end_date ."' AND metode != '1-1006'";
                            }
                            elseif ($tanggal == "Bulan") {
                                $filter = "WHERE month(tgl_transaksi) = '". date("m") ."'";
                                $filter2 = "WHERE month(a.tgl_invoice) = '". date("m") ."' AND a.jatuh_tempo != 0";
                                $filter3 = "WHERE month(tgl_terima) = '". date("m") ."' AND metode != '1-1006'";
                            }
                            ...
                            $piutang = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(subtotal) AS subtotal, a.jatuh_tempo FROM tb_barang_keluar a $filter2 "));

                            $piutang2 = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(pay) AS pay FROM inv_payment $filter3 "));
                                                ...
                                                <tbody>
                                                    <?php
                                                        ...
                                                        if (isset($_POST['search'])) {
                                                            $tanggal = $_POST['tanggal'];
                                                            if ($tanggal == "Hari") {
                                                                $filter_tabel = "WHERE tgl_transaksi = '". date("Y-m-d") ."'";
                                                                $filter_tabel_2 = "AND tgl_transaksi = '". date("Y-m-d") ."'";
                                                            }
                                                            elseif ($tanggal == "Minggu") {
                                                                $start_date = date("Y-m-d", strtotime('last monday'));
                                                                $end_date = date("Y-m-d", strtotime('next sunday'));
                                                                $filter_tabel = "WHERE tgl_transaksi BETWEEN '". $start_date ."' AND '". $end_date ."'";
                                                                $filter_tabel_2 = "AND tgl_transaksi BETWEEN '". $start_date ."' AND '". $end_date ."'";
                                                            }
                                                            elseif ($tanggal == "Bulan") {
                                                                $filter_tabel = "WHERE month(tgl_transaksi) = '". date("m") ."'";
                                                                $filter_tabel_2 = "AND month(tgl_transaksi) = '". date("m") ."'";
                                                            }

                                                        }
                                                        $customer = mysqli_query($conn, "SELECT a.id_customer,SUM(total) as total,b.nama_customer FROM tb_barang_keluar a
                                                        JOIN tb_customer AS b ON a.id_customer = b.id_customer $filter_tabel GROUP BY id_customer ORDER BY total DESC LIMIT 6");
                                            ...
                                            <table class="table table-hover table-sm w-100 display tb_jps_re_ins tb_jps_ins mt-15">
                                                ...
                                                <tbody>
                                                    <?php
                                                        ...
                                                        JOIN tb_customer AS b ON a.cust_invoice = b.id_customer WHERE a.jatuh_tempo != 0 $filter_tabel_2 GROUP BY cust_invoice ORDER BY subtotal_invoice DESC LIMIT 6");
                                                        while ($row = mysqli_fetch_assoc($customer)) {
                                                            echo '
                                                                <tr>
                                                                    <td class="text-center fs-11">'. $row['nama_customer'] .'</td>
        ...
        data: {
            labels: [
                ...
                if (isset($_POST['search'])) {
                    $tanggal = $_POST['tanggal'];
                    if ($tanggal == "Hari") {
                        $filter_pie_1 = "WHERE tgl_transaksi = '". date("Y-m-d") ."'";
                    }
                    elseif ($tanggal == "Minggu") {
                        $start_date = date("Y-m-d", strtotime('last monday'));
                        $end_date = date("Y-m-d", strtotime('next sunday'));
                        $filter_pie_1 = "WHERE tgl_transaksi BETWEEN '". $start_date ."' AND '". $end_date ."'";
                    }
                    elseif ($tanggal == "Bulan") {
                        $filter_pie_1 = "WHERE month(tgl_transaksi) = '". date("m") ."'";
                    }
                    
                }
                    $sql_tertinggi = mysqli_query($conn, "SELECT a.id_bahan,a.qty * a.berat, b.nama_bahan FROM tb_barang_keluar a 
                    JOIN tb_bahan AS b ON a.id_bahan = b.id_bahan $filter_pie_1 ORDER BY qty * berat DESC LIMIT 6");
            ...
            data: [
                <?php
                $sql_tertinggi = mysqli_query($conn, "SELECT id_bahan,qty * berat,qty FROM `tb_barang_keluar` $filter_pie_1 ORDER BY `qty * berat` DESC LIMIT 6");
                while ($row = mysqli_fetch_assoc($sql_tertinggi)) {
                    echo $row['qty * berat'];
        ...
        data: {
            labels: [
                <?php
                    ...
                    JOIN tb_bahan AS b ON a.id_bahan = b.id_bahan $filter_pie_1  ORDER BY qty DESC LIMIT 6");
                    while ($row = mysqli_fetch_assoc($sql_tertinggi)) {
                        echo '"'.$row['nama_bahan'].'"' . ",";
                    }
            ...
            data: [
                <?php
                $sql_tertinggi = mysqli_query($conn, "SELECT qty FROM tb_barang_keluar $filter_pie_1 ORDER BY qty DESC LIMIT 6");
                while ($row = mysqli_fetch_assoc($sql_tertinggi)) {
                    echo $row['qty'];
                    echo ",";
                }
